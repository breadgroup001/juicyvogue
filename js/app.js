...
// Live countdown to female collection launch
  const target = new Date();
  target.setDate(target.getDate() + 29);
  target.setHours(target.getHours() + 10, target.getMinutes() + 15, 34, 0);
...
function renderFilterBar() {
    ...
    const categories = new Set(['Trending']);
    allProducts.forEach(record => {
        if (record.fields.trends) {
            categories.add(record.fields.trends);
        }
    });

    filterBar.innerHTML = '';
    categories.forEach(trends => {
        const button = document.createElement('button');
        button.className = `filter-btn ${trends === currentCategory ? 'active' : ''}`;
        button.textContent = trends;
        button.onclick = () => {
...
function renderShopGrid() {
    ...
    const filteredProducts = allProducts.filter(record => {
        ...
        return record.fields.trends === currentCategory;
    });
    ...
    contentContainer.innerHTML = `
        ...
        <div class="details-content-column">
            ...
            <div>
                ...
                <p class="redirect-notice"><span><i class="bx bx-truck"></i>Global Shipping</span><span><i class="bx bx-shield"></i>Authentic Craft</span><span><i class="bx bx-refresh-ccw"></i>30 Day Returns</span></p>
            </div>
        </div>
    `;
